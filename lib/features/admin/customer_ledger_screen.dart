...
class CustomerLedgerScreen extends ConsumerWidget {
                  ...
                  content: Column(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(
                            labelText: 'Reference (e.g. Cash, UTR)'),
                      ),
                    ],
                  ),
                  actions: [
                    ...
                    ElevatedButton(
                      onPressed: isLoading ? null : () async {
                        final amount = double.tryParse(
                            amountController.text.trim()) ?? 0.0;
                        if (amount > 0) {
                          ...
                          try {
                            await ref.read(paymentServiceProvider).addPayment(
                                ...
                                referenceController.text
                                    .trim()
                                    .isEmpty ? 'Cash' : referenceController.text
                                    .trim()
                            );
  ...
  void _showEditCreditLimitDialog(BuildContext context, WidgetRef ref) {
                            ...
                            if (context.mounted) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text(
                                      'Credit Limit Updated! Please refresh screen to see changes.')));
                            }
                          } catch (e) {
          ...
          slivers: [
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(
                            icon: const Icon(Icons.edit, size: 16, color: Colors
                                .grey),
                            onPressed: () =>
                                _showEditCreditLimitDialog(context, ref)),
                      ],
                    ),
                    Text('Limit: ${currencyFormatter.format(
                        customer.creditLimit)}', style: const TextStyle(
                        color: Colors.grey, fontSize: 12)),
                    const SizedBox(height: 8),
                    Text(currencyFormatter.format(customer.creditBalance),
            ...
            const SliverToBoxAdapter(
              child: Padding(
                ...
                child: Text('Recent Payments', style: TextStyle(
            ...
            paymentsAsync.when(
              ...
              data: (payments) {
                if (payments.isEmpty) return const SliverToBoxAdapter(
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate(
                      ...
                      return ListTile(
                        leading: const CircleAvatar(backgroundColor: Colors
                            .green, child: Icon(
            ...
            ordersAsync.when(
              ...
              error: (e, st) =>
                  SliverToBoxAdapter(child: Center(child: Text('Error: $e'))),
              data: (orders) {
                final customerOrders = orders.where((o) =>
                o.retailerId == customer.id).toList();
                if (customerOrders.isEmpty) return const SliverToBoxAdapter(
                    child: Padding(padding: EdgeInsets.all(16),
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate(
                      ...
                      return ListTile(
                        leading: const CircleAvatar(backgroundColor: Colors
                            .redAccent, child: Icon(
                            Icons.arrow_upward, color: Colors.white)),
                        title: Text(currencyFormatter.format(o.grandTotal),
                            style: const TextStyle(
                                fontWeight: FontWeight.bold)),
                        subtitle: Text('${DateFormat('dd MMM yyyy').format(
                            o.date)} • Status: ${o.status.name}'),
                      );
                    },
